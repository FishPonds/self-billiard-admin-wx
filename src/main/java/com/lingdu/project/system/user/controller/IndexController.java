...
public class IndexController extends BaseController {
    ...
    // 系统首页
    @PostMapping("/index")
    public String index(ModelMap mmap, @RequestParam("selectedStoreId") String selectedStoreId, HttpSession session) {
        // 取身份信息
        SelfUser user = getSelfUser();
        // 根据用户id取出菜单
        List<SelfMenu> menus = selfMenuService.selectMenusBySelfUser(user);
        session.setAttribute("storeId", Long.parseLong(selectedStoreId));
        mmap.put("storeId", selectedStoreId);
    ...
    public String storeInformation(ModelMap modelMap) {
        //获取当前登录用户
        SelfUser selfUser = getSelfUser();
        SelfStoreInformation selfStoreInfo = new SelfStoreInformation();
        selfStoreInfo.setSelfUserId(selfUser.getSelfUserId());
